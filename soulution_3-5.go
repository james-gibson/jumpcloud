...
func processJobInQueue(id int, jobs chan hashJob, completedJobs chan completedHashJob) {
    //fmt.Println("Worker ",id," has launched")
    go func () {
        for j := range jobs {
            ...
            // this comes back as nanoseconds, converting into milliseconds
            timing := time.Now().Sub(start) / 1000000
